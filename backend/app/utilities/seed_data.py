...
def seed_admin_user() -> None:
  ...
  Always resets the bootstrap admin password to AdminPass1 so deploys recover
  from corrupt hashes without manual DB surgery.
  """
  email = "admin@pinpoint.local"
  password_hash = bcrypt.hashpw(b"AdminPass1", bcrypt.gensalt()).decode("utf-8")
  user = User.query.filter_by(email=email).first()
  if user:
    user.full_name = user.full_name or "PINPOINT Administrator"
